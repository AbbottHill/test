...
public class JedisUtil {
    ...
    private static int redisPartition = 1;

    @Before
    public void init() {
        try {
            jedisConfig.load(JedisUtil.class.getResourceAsStream("/redis.properties"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        jedis = JedisUtil.getJedis(jedisConfig.getProperty("REDIS_IP"), Integer.parseInt(jedisConfig.getProperty("REDIS_PORT")));
    }
...
//        System.out.println("清空数据："+jedis.flushDB());
        ...
        System.out.println("新增<'username','小明'>的键值对：" + jedis.set("username", "小明"));
     ...
     */
    @Test
    public void incrFunc() {
        String xfj = "340101";
//        jedis.set(xfj, "90000000");
        jedis.incr(xfj);
        System.out.println(jedis.get(xfj));
    ...
    @After
    public void close() {
        JedisUtil.closeJedis(jedis, jedisConfig.getProperty("REDIS_IP"), Integer.parseInt(jedisConfig.getProperty("REDIS_PORT")));
    }
...
//            config.setMaxActive(RedisConfig.MAX_ACTIVE);
            config.setMaxIdle(Integer.parseInt(jedisConfig.getProperty("MAX_IDLE")));
//            config.setMaxWait(RedisConfig.MAX_WAIT);
            config.setTestOnBorrow(true);
            config.setTestOnReturn(true);

            pool = new JedisPool(config, ip, port, Integer.parseInt(jedisConfig.getProperty("TIMEOUT")), jedisConfig.getProperty("REQUIREPASS"));
            maps.put(key, pool);
        } else {
            pool = maps.get(key);
        }
    ...
    public static Jedis getJedis(String ip, int port) {
        ...
        } while (jedis == null && count < Integer.parseInt(jedisConfig.getProperty("RETRY_NUM")));
        return jedis;
    }
